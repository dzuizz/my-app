      ...
      <ul>
        {achievements.map((grp, index) => (
          <li className="text-slate-400" key={index}>
            In <strong className="text-slate-50">{grp.year}</strong>, I was in <strong className="text-slate-50">{grp.grade}</strong> and got:<br />
            <ul>
              {grp.items.map((item, index) =>
                <li key={index}>
                  <br />
                  <strong className="text-slate-50">{item.title}</strong>{" "}
                  <i>{contests[item.title] ? `(${contests[item.title]})` : ``}</i>{" - "}
